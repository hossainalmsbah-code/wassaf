...
function buildMarketingPrompt({ productName, audience, features, price, brandTone, longDescription }) {
  const user = [
    ...
    `الخصائص: ${features}`,
    price ? `السعر (للسياق فقط، ما تذكره بالنص المكتوب): ${price}` : '',
    ...
    price ? 'قاعدة إلزامية: لا تذكر رقم السعر ولا كلمة "ريال" بأي نص تكتبه — السعر معلومة خلفية بس لتحديد النبرة المناسبة.' : ''
  ].filter(Boolean).join('\n');
...
${price ? 'استخدم السعر بس كمعلومة خلفية تساعدك تحدد نبرة الكتابة (فاخر/اقتصادي/متوسط) — لا تذكر رقم السعر ولا كلمة "ريال" ولا أي إشارة صريحة للسعر داخل أي نص تكتبه (long, short, seo, seoMeta). السعر يظهر تلقائياً بصفحة المنتج بمتجر التاجر، وذكره بالوصف نفسه يسبب تعارض لو غيّر التاجر السعر لاحقاً.' : 'ما فيه سعر مُدخل — لا تخترع أو تفترض أي رقم سعر بالنص.'}
...
3. "seo": جملة واحدة قصيرة (لا تتجاوز 160 حرف) محسّنة لظهور المنتج بجوجل، تتضمن اسم المنتج وأهم ميزة فيه. (هذا الحقل يبقى زي ما هو، يُستخدم كوصف عام).
4. "seoTitle": عنوان SEO منفصل ومختلف عن اسم المنتج الأصلي، بين 40 و60 حرف بالضبط، يتضمن اسم المنتج وأقوى ميزة أو كلمة بحث مهمة.
5. "seoMeta": وصف Meta منفصل، بين 130 و160 حرف بالضبط، يشرح المنتج بأسلوب يشجع على الضغط عليه بنتائج البحث.
6. "seoKeywords": مصفوفة من 5 إلى 8 كلمات أو عبارات مفتاحية قصيرة (كل وحدة كلمتين إلى ثلاث كلمات كحد أقصى) يبحث فيها عميل يدور على هذا المنتج تحديداً.
